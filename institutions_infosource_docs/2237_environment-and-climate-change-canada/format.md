# Format Guide: Environment and Climate Change Canada

French name: Environnement et Changement climatique Canada
...
And the value resolves to a bank id like `EC PPU 010` (`<ORG> <SERIES> <3 digits>`).
...
1. Normalize join key to bank series + number (for example `PPU 010`) and use this to pair EN/FR rows.
2. Set `bank_number_key` from normalized full key (`EC PPU 010` style).
...
- No label leakage between fields after extraction.
