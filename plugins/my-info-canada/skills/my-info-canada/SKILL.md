...
3. Ask one `next_step.prompt` at a time. For a `question` step, map the response only to `yes`, `no`, `not_sure`, or `prefer_not_to_answer`. For a `refinement` step, map the response only to one or more returned option codes. For a `timing` step, map it only to a returned timing kind or an approximate year. For a `department` step, let the person select one or more returned institution IDs; never ask them to narrate case details.
4. Send controlled question answers in `answers`. Send adaptive selections and their per-option timing in `refinements`, and department selections in `departments`. A correction replaces the earlier controlled value; never edit state by hand.
...
7. When `complete` is true, call `my_info_evaluate` with `include_possible=false` and no more than 50 results. Group results by holding status, disclose any `inventory_gaps`, and say they are candidates rather than confirmation. For every displayed candidate, include `categories_of_personal_information` and, when present, `specific_information_types`; these describe possible data content, not confirmation that the institution holds it about the person. Ask before including possible/review matches or fetching another page.
