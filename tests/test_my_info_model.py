...
class MyInfoModelTests(unittest.TestCase):
    def test_loads_both_pib_scopes_with_unique_ids(self):
        ...
        self.assertEqual(1005, len(records))
        self.assertEqual(1005, len({record.record_id for record in records}))
        self.assertEqual(49, sum(record.scope == "standard" for record in records))
        self.assertEqual(956, sum(record.scope == "institution" for record in records))
